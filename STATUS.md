...
| wire decode: sub-messages | PARITY-SEALED | courts/decode-submsg (142 cases, 0 residuals; merge, repeated, nested, recursive, depth limits) |
| wire decode: maps         | PARITY-SEALED | courts/decode-submsg (142 cases, 0 residuals; map fields, last-wins, unknown-entry re-encode, message values, nested maps) |
...
decode-known (517), decode-submsg (142), arena (61), and collections (52).
...
upb_Map. Phase 2 (binary wire parity) is underway: map fields decode is
sealed inside the decode-submsg court (last-wins inserts, empty entries,
string keys/values, message values, nested maps, unknown-inside-entry
re-encode via AddMapEntryUnknown, depth boundaries). Next milestones per
forensics/OPEN_QUESTIONS.md: groups, closed enums, then the encoder court.
